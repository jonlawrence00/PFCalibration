#include <stdio.h>
// #include<conio.h>
void genrateplot_1dresponsevsE_trans(TString legendname, TString plot_name, TString path1, TString plot_name2,TString region, int xmin=20, int xmax=24)
{
  //   gStyle->SetOptStat(0);
  ...
  //  for(int i=4; i<50;i++)
	...
	sprintf(hist_name,"Valid_norm_pred_eta_2.500000_2.750000_E_%d_%d",xmin,xmax);
        ...
        sprintf(hist_name,"Valid_norm_pred_eta_1.260000_1.680000_E_%d_%d",xmin,xmax);
        sprintf(hist_name2,"Train_norm_pred_eta_1.260000_1.680000_E_%d_%d",xmin,xmax);
      }
      else{
	...
	sprintf(hist_name2,"Train_norm_pred_trueEn_TrueEn_%d_to_%d",xmin,xmax);
     ...
     //      ptstats1->SetOptStat(0000);
      ...
      TPaveStats *ptstats2 = new TPaveStats(0.60,0.48,0.9,0.67,"brNDC");
      ...
      TLegend* legends = new TLegend(0.1, 0.7, 0.45, 0.8,"","brNDC"); // the numbers determine the position of the box 
