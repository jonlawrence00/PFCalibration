import bw_conv_cb.python.breitWigner as breitWigner
import bw_conv_cb.python.crystalBall as crystalBall
import numpy as np
...
def fit(x,y):
    ...
    uncertainties = np.zeros((5,5))

    mu = 91.188+result.x[2]
    dmu =  min(np.abs(uncertainties[0][0]),np.abs(uncertainties[0][1]))

    sigma = result.x[3]
    dsigma = min(np.abs(uncertainties[1][0]),np.abs(uncertainties[1][1]))
    ...
    return mu-91.188, dmu, sigma, dsigma, chi_sqr
