...
#bin_range =[0,0.5,1.0,1.5,2.0,2.5,3,3.5]
#bin_range = np.arange(0.00,4.0,0.42)

#bin_range2 =[0.0001,0.42,0.84,1.26,1.68,2.1,2.52,2.94,3.36]
#print(bin_range)
#print("Total bins : ", len(bin_range))
bin_range =[0.0,1.55,2.5,2.75,3.0]
print(bin_range)
print("Total bins : ", len(bin_range))


#Ebin_range =[0, 2, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 104, 114, 124, 134, 144, 154, 164, 174, 184, 194, 204, 214, 224, 234, 244, 254, 264, 274, 284, 294, 304, 314, 324, 334, 344]
#Ebin_range2 =[2, 8, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 104,108, 124, 134, 144, 154, 164, 174, 184, 194, 204, 214, 224, 234, 244, 254, 264, 274, 284, 294, 304, 314, 324, 334, 344, 354]

Ebin_range =[8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 104, 114, 124, 134, 144, 154, 164, 174, 184, 194, 204, 214, 224, 234, 244, 254, 264, 274, 284, 294, 304, 314, 324, 334, 344]
Ebin_range2 =[16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92, 96, 100, 104,108, 124, 134, 144, 154, 164, 174, 184, 194, 204, 214, 224, 234, 244, 254, 264, 274, 284, 294, 304, 314, 324, 334, 344, 354]
...
hist2D_norm_Eta_predTrue_Train=ROOT.TProfile2D('hist2D_norm_Eta_predTrue_Train', """:"(pred-true)/true in GeV":""", 8, 0.00,3.36,10,0,400)
hist2D_norm_Eta_predTrue_Valid=ROOT.TProfile2D('hist2D_norm_Eta_predTrue_Valid', """:"(pred-true)/true in GeV":""", 8, 0.00,3.36,10,0,400)
hist_pred_Valid=ROOT.TH1F('hist_pred_Valid', """:"Predictions in valid set":""", 800, 0, 400)
hist_pred_Train=ROOT.TH1F('hist_pred_Train', """:"Predictions in train set":""", 800, 0, 400)
hist_true_Valid=ROOT.TH1F('hist_true_Valid', """:"True E in valid set":""", 800, 0, 400)
hist_true_Train=ROOT.TH1F('hist_true_Train', """:"True E in train set":""", 800, 0, 400)
...
for i in range(len(valid_idx)):
    if(ecal_pkl[valid_idx[i]]>0):
        ...
        for Ebin in range(len(Ebin_range)-1):
            hist_true_Valid.Fill(valid_trueEn)
            hist_pred_Valid.Fill(valid_predEn)
...
for i in range(len(train_idx)):
    if(ecal_pkl[train_idx[i]]>0):
        ...
        predEn=np.empty(len(bin_range)-1,dtype='float')
        for Ebin in range(len(Ebin_range)-1):
            hist_true_Train.Fill(train_trueEn)
            hist_pred_Train.Fill(train_predEn)
        for ibin in range(len(bin_range)-1):
...
hist_true_Train.Write()
hist_pred_Train.Write()
hist_true_Valid.Write()
hist_pred_Valid.Write()
