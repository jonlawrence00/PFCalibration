...
import os
from training.semiparam import dscb_semiparam_sigmoid, dscb_semiparam, dscb_semiparam_sigmoid_minalpha
#sys.exit()
...
class Train:
    def __init__(self, folder=None, data_folder=None, weights_name = None,
            ...
            graph_features = []):
        self.folder = folder
        self.data_folder = '%s'%data_folder
    ...
    def train(self):
        ...
        trainer.build_model(name='DynamicReductionNetwork', loss_func= self.loss_func,
                ...
                thresh = self.thresh, reg = self.reg, epsilon = self.epsilon, minalpha = self.minalpha,
    ...
    def predict(self):
        ...
        if self.semiparam:
            ...
            if self.loss_func == 'dscb_loss_sigmoid':
                semifunc = lambda x: dscb_semiparam_sigmoid(x, np.log(self.thresh), self.epsilon)
            elif self.loss_func == 'dscb_loss_sigmoid_minalpha':
                semifunc = lambda x: dscb_semiparam_sigmoid_minalpha(x, np.log(self.thresh), self.epsilon, self.minalpha)
...
#                    x = x.detach().cpu().numpy()
        ...
        if self.latent_probe is not None:
            ...
            predname = '%s/pred_tb.pickle'%(self.folder)
