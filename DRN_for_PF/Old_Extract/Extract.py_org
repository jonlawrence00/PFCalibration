...
class Extract:
    ...
    def readHGCAL(self):
        ...
        Hit_X = self.tree['comb_rechit_x_trimAhcal'].array()#combined_rechit_x'].array()
        print("\ttook %0.3f seconds"%(time()-t0))
        
        t0=time()
        print("Reading rechit_y...")
        Hit_Y = self.tree['comb_rechit_y_trimAhcal'].array()#combined_rechit_y'].array()
        print("\ttook %0.3f seconds"%(time()-t0))
        
        t0=time()
        print("Reading rechit_z...")
        Hit_Z_orig = self.tree['comb_rechit_z_trimAhcal'].array()#combined_rechit_z'].array()
        print("\ttook %0.3f seconds"%(time()-t0))
        t0=time()
        print("Reading rechit_z...")
        Hit_Z = self.tree['comb_rechit_z_trimAhcal'].array()
        print("\ttook %0.3f seconds"%(time()-t0))

        t0=time()
        print("Reading rechit_energy...")
        recHitEn = self.tree['rechitEn_trimAhcal'].array()#combined_rechits_energy'].array()
        print("\ttook %0.3f seconds"%(time()-t0))
